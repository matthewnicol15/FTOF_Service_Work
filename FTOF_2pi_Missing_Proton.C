// ECAL cuts remove last 2 counters for FTOF1A and FTOF2
...
void FTOF_2pi_Missing_Proton(){
  ...
  TFile fileOutput1("/volatile/clas12/matthewn/FTOF/FTOF_Efficiency_RGA_FALL2018_skim4_Outbending_2pi_missproton_allcuts_07092021_01.root","recreate");
  ...
  Int_t negative, positive, nonpip;
  ...
  for(Int_t i=0;i<files->GetEntries();i++){
    ...
    while(c12.next()==true){
      ...
      if(electrons.size() == 1 && pips.size() == 1 && pims.size() == 1 && negative == 2 && positive == 2){
        ...
        hmass->Fill(missproton.M());

        DeltaP = missproton.Rho() - proton.Rho();
        DeltaTheta = TMath::RadToDeg()* (missproton.Theta() - proton.Theta());
        DeltaPhi = TMath::RadToDeg()* (missproton.Phi() - proton.Phi());

        // Checking the background after cuts
        if(fabs(DeltaP) < 0.3 && fabs(DeltaTheta) < 10 && fabs(DeltaPhi) < 10 ){

          hmass_cuts->Fill(missproton.M());
        }

        // Cut on missing mass of the pi^-
        if(missproton.M() < 1.038 && missproton.M()>0.838){
          ...
          if(fabs(DeltaP) < 0.3 && fabs(DeltaTheta) < 10 && fabs(DeltaPhi) < 10 ){
            ...
            for(auto& p : particles){
              ...
              switch(p->getRegion()) {
                ...
                Calorimeter_Hits = (Status / 10)%10; // Getting the hits in calorimeter
                ...
                if(PCAL_Energy > 0 && ECIN_Energy > 0){
                  ...
                  if(p->traj(FTOF,FTOF2)->getDetector()==12 && p->traj(FTOF,FTOF2)->getLayer()==3){
                    ...
                    // cout<<x_2<<" "<<y_2<<" "<<z_2<<" "<<L_2<<" "<<L_det_2<<" "<<L_Perp_2<<" "<<alpha_2<<endl;
                    // Determine which sector the hit is in using alpha
                    // Look at positive x (sectors 1,2 and 6)
                    if(x_2 > 0){
                      // Sector 2
