      ...
      for(Int_t i_charge = charge_min; i_charge < charge_max; i_charge++){
        ...
        for(Int_t i_sector = 0; i_sector < 6; i_sector++){
          ...
          if(i_topology == 4 && (i_detector == 0 || i_detector == 1)){
            ...
            // calorimeter
            TH2F *pi_Denominator = (TH2F*) f1->Get(pi_Denominator_name_stream.str().c_str());
            TH2F *pi_Numerator = (TH2F*) f1->Get(pi_Numerator_name_stream.str().c_str());

            // Take the 1D projection
            pi_Denominator_y[i_detector][i_charge][i_sector] = (TH1F*) pi_Denominator->ProjectionX();
            pi_Numerator_y[i_detector][i_charge][i_sector] = (TH1F*) pi_Numerator->ProjectionX();

            // Remove the 2D histograms once the 1D projections are taken
