      ...
      switch(p->getRegion()) {
         ...
         if(p->traj(FTOF,FTOF1A)->getDetector()==12 && p->traj(FTOF,FTOF1A)->getLayer()==1){
            ...
            if(p->sci(FTOF1A)->getEnergy()){
               ...
               if(i_topology==3) h_Energy_Dep[i_topology][i_detector][i_charge][i_sector]->Fill(Component,p->sci(FTOF1A)->getEnergy());
            }
            ...
            if(x_1a > 0){
               ...
               if(p->sci(FTOF1A)->getEnergy()) h_Numerator[i_topology-1][i_detector][i_charge][i_sector - 1]->Fill(L_det_1a, L_Perp_1a);
            }
            ...
            if(p->traj(FTOF,FTOF1B)->getDetector()==12 && p->traj(FTOF,FTOF1B)->getLayer()==2){
               ...
               if(x_1b > 0){
                  ...
                  // Check if there is energy deposited on the scintillator
                  if(p->sci(FTOF1B)->getEnergy()) h_Numerator[i_topology-1][i_detector][i_charge][i_sector - 1]->Fill(L_det_1b, L_Perp_1b);
               }
            ...
            if(p->traj(FTOF,FTOF2)->getDetector()==12 && p->traj(FTOF,FTOF2)->getLayer()==3){
               ...
               if(x_2 > 0){
                  ...
                  // Check if there is energy deposited on the scintillator
                  if(p->sci(FTOF2)->getEnergy()) h_Numerator[i_topology-1][i_detector][i_charge][i_sector - 1]->Fill(L_det_2, L_Perp_2);
               }
      ...
      for(Int_t i_detector=0;i_detector<3;i_detector++){
         ...
         for(Int_t i_charge=0;i_charge<2;i_charge++){
            ...
            for(Int_t i_sector=0;i_sector<6;i_sector++){
               ...
               Energy_name_stream<<"h_Energy_Topo_"<<i_topo<<"_Det_"<<i_detector<<"_Charge_"<<i_charge<<"_Sec_"<<i_sector;
               ...
               if (i_detector==0){
                  ...
                  h_Energy_Dep[i_topo][i_detector][i_charge][i_sector] = new TH2F(Energy_name_stream.str().c_str(),"", 25,1,23,200,0,100);
