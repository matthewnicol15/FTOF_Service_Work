      ...
      for(Int_t i_detector = detector_min; i_detector < detector_max; i_detector++){
         ...
         for(Int_t i_charge = charge_min; i_charge < charge_max; i_charge++){
            ...
            for(Int_t i_sector = 0; i_sector < 6; i_sector++){
               ...
               Numerator_y_copy[i_topology][i_detector][i_charge][i_sector] = (TH1F*)Numerator_y[i_topology][i_detector][i_charge][i_sector]->Clone();
   ...
   for(Int_t i_detector = 0; i_detector < 3; i_detector++){
      ...
      for(Int_t i_charge = 0; i_charge < 2; i_charge++){
         ...
         for(Int_t i_sector = 0; i_sector < 6; i_sector++){
               ...
               if(i_detector == 0){
                  ...
                  for(Int_t bin_0 = 2; bin_0 < 25; bin_0++){
                     ...
                     if(bin_0 < 24){
                        ...
                        (bin_0,Numerator_y_copy[3][i_detector][i_charge][i_sector]->GetBinContent(bin_0));
                     }
                     ...
                     // is used
                        ...
                        if(i_charge==0){
                           ...
                           Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                           (bin_0,Numerator_y_copy[0][i_detector][i_charge][i_sector]->GetBinContent(bin_0));
                        }
                        else{
                           // Checking charge to determine which 2pi topology to use
                           Overall_Denominator_y[i_detector][i_charge][i_sector]->SetBinContent
                           ...
                           // Getting numerator values from 2pi
                           Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                           (bin_0,Numerator_y_copy[1][i_detector][i_charge][i_sector]->GetBinContent(bin_0));
                        }
                     }
                  }
               }
               ...
               if(i_detector == 1){
                  ...
                  for(Int_t bin_1 = 2; bin_1 < 64; bin_1++){
                     ...
                     if(bin_1 < 59){
                        ...
                        // Getting numerator values from single track
                        Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                        (bin_1,Numerator_y_copy[3][i_detector][i_charge][i_sector]->GetBinContent(bin_1));
                     }
                        ...
                        if(i_charge==0){
                           ...
                           // Getting numerator values from 2pi
                           Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                           (bin_1,Numerator_y_copy[0][i_detector][i_charge][i_sector]->GetBinContent(bin_1));
                        }
                        else{
                           // Checking charge to determine which 2pi topology to use
                           Overall_Denominator_y[i_detector][i_charge][i_sector]->SetBinContent
                           ...
                           // Getting numerator values from 2pi
                           Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                           (bin_1,Numerator_y_copy[1][i_detector][i_charge][i_sector]->GetBinContent(bin_1));
                        }
                     }
            ...
            if(i_detector == 2){
               ...
               for(Int_t bin_2 = 2; bin_2 < 7; bin_2++){
                  ...
                  if(i_charge == 0){
                     ...
                     Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                     (bin_2,Numerator_y_copy[0][i_detector][i_charge][i_sector]->GetBinContent(bin_2));
                  }
                  ...
                  // For positive efficieny get the pi+ results
                     ...
                     // Getting numerator from 2pi
                     Overall_Numerator_y[i_detector][i_charge][i_sector]->SetBinContent
                     (bin_2,Numerator_y_copy[1][i_detector][i_charge][i_sector]->GetBinContent(bin_2));
                  }
               }
            }
            ...
            Overall_Numerator_y[i_detector][i_charge][i_sector]->SetTitle(Efficiency_title_stream.str().c_str());
