...
void FTOF_2pi_Missing_Pip(){
  ...
  TFile fileOutput1("/volatile/clas12/matthewn/FTOF/FTOF_Efficiency_RGA_FALL2018_skim4_Outbending_2pi_misspip_allcuts_07092021_01.root","recreate");
  ...
  Int_t negative, positive, nonproton;
  ...
  for(Int_t i=0;i<files->GetEntries();i++){
    ...
    while(c12.next()==true){
      ...
      if(electrons.size() == 1 && protons.size() == 1 && pims.size() == 1 && negative == 2 && positive == 2){
        ...
        // Calculating difference between detected and reconstructed pi+
